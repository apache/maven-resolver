...
public class PartialFileTest {
    ...
    private static class StubRemoteAccessChecker implements PartialFile.RemoteAccessChecker {
    ...
    private static class ConcurrentWriter extends Thread {
        ...
        public ConcurrentWriter(File dstFile, int sleep, int length) throws InterruptedException {
    ...
    private PartialFile newPartialFile(long resumeThreshold, int requestTimeout) throws Exception {
    ...
    @Test
    public void testCloseNonResumableFile() throws Exception {
        PartialFile partialFile = newPartialFile(-1, 100);
        assertNotNull(partialFile);
        assertNotNull(partialFile.getFile());
        assertTrue(
                partialFile.getFile().getAbsolutePath(), partialFile.getFile().isFile());
        partialFile.close();
        assertFalse(
                partialFile.getFile().getAbsolutePath(), partialFile.getFile().exists());
    }

    @Test
    public void testCloseResumableFile() throws Exception {
    ...
    @Test
    public void testResumableFileCreationError() throws Exception {
    ...
    @Test
    public void testResumeThreshold() throws Exception {
        PartialFile partialFile = newPartialFile(0, 100);
        assertNotNull(partialFile);
    ...
    @Test(timeout = 10000L)
    public void testResumeConcurrently_RequestTimeout() throws Exception {
    ...
    @Test(timeout = 10000L)
    public void testResumeConcurrently_AwaitCompletion_ConcurrentWriterSucceeds() throws Exception {
        assumeTrue(PROPER_LOCK_SUPPORT);
    ...
    @Test(timeout = 10000L)
    public void testResumeConcurrently_AwaitCompletion_ConcurrentWriterFails() throws Exception {
        assumeTrue(PROPER_LOCK_SUPPORT);
        assertTrue(dstFile.setLastModified(System.currentTimeMillis() - 60L * 1000L));
        ConcurrentWriter writer = new ConcurrentWriter(dstFile, 100, -10);
    ...
    @Test(timeout = 10000L)
    public void testResumeConcurrently_CheckRemoteAccess() throws Exception {
        assumeTrue(PROPER_LOCK_SUPPORT);
