...
class SimpleLocalRepositoryManager implements LocalRepositoryManager {
     ...
     * {@code true}, in which case this method creates unique identifier based on ID and current configuration of the
     * remote repository (as it may change).
     */
    protected String getRepositoryKey(RemoteRepository repository, String context) {
        String key;

        if (repository.isRepositoryManager()) {
            // repository serves dynamic contents, take request parameters into account for
            // key
    ...
    public LocalArtifactResult find(RepositorySystemSession session, LocalArtifactRequest request) {
        ...
        // Local repository CANNOT have timestamped installed, they are created only
        // during deploy
