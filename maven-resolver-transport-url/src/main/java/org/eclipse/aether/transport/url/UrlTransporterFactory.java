...
/**
 * A bootstrapping transporter factory for repositories using the {@code http:} or {@code https:} protocol.
 *
 * @since 2.0.21
 */
...
public final class UrlTransporterFactory implements HttpTransporterFactory {
    ...
    @Inject
    public UrlTransporterFactory(ChecksumExtractor checksumExtractor, PathProcessor pathProcessor) {
    ...
    @Override
    public UrlTransporter newInstance(RepositorySystemSession session, RemoteRepository repository)
        ...
        return new UrlTransporter(repository, session, checksumExtractor, pathProcessor);
    }
}
