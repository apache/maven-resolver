...
public class ChecksumFailureException extends RepositoryException {
    ...
    public ChecksumFailureException(String expected, String expectedKind, String actual) {
        super("Checksum validation failed, expected '" + expected + "'"
                + (expectedKind == null ? "" : " (" + expectedKind + ")") + " but is actually '" + actual + "'");
    ...
    /**
     * Creates a new exception with the specified detail message. The resulting exception is not {@link #isRetryWorthy()
     * retry-worthy}.
     *
