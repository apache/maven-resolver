 ...
 * <p>
 * Note: the "trusted" meaning depends solely on implementation and the user using it. Resolver itself does nothing for
 * "trust" (like some crypto magic or what not). It all boils down that the source being used by implementation is
...
public interface TrustedChecksumsSource {
     ...
     * artifact. Empty map means in this case "no information", but how that case is interpreted depends on consumer for
     * trusted checksums.
     *
    ...
    /**
     * A writer that is able to write/add trusted checksums to this implementation. Should be treated as a resource as
     * underlying implementation may rely on being closed after not used anymore.
     */
    interface Writer extends Closeable {
        /**
         * Performs whatever implementation requires to "set" (write/add/append) given map of trusted checksums. The
         * passed in list of checksum algorithm factories and the map must have equal size and mapping must contain all
         * algorithm names in list.
         */
    ...
    /**
     * Some trusted checksums sources may implement this optional method: ability to write/add checksums to them. If
     * source does not support this feature, method should return {@code null}.
     */
