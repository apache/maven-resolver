...
/**
 * A simple tool to check file locking on your OS/FS/Java combo. To use this tool, just copy it it baseDir on
                    ...
                    StandardOpenOption.WRITE ) )
            {
                try ( FileLock latchLock = latchChannel.lock( 0L, 1L, false ) )
                {
                    new Thread( () ->
                    {
                        try
                        {
                            oneResult.set( new ProcessBuilder( javaCmd, TestNioLock.class.getName(),
                                    "perform", args[1], args[2] ).inheritIO().start().waitFor() );
                        }
                        catch ( Exception e )
                        {
                    ...
                    new Thread( () ->
                    {
                        try
                        {
                            twoResult.set( new ProcessBuilder( javaCmd, TestNioLock.class.getName(),
                                    "perform", args[1], args[2] ).inheritIO().start().waitFor() );
                        }
                        catch ( Exception e )
                        {
