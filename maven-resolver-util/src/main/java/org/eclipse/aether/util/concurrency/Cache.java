    ...
    class RefConcurrentMap<K, V> implements Map<K, V>, Cache<K, V> {
        ...
        private abstract static class RefConcurrentReference<T> {
            ...
            public boolean equals(Object obj) {
                ...
                if (!(obj instanceof RefConcurrentMap.RefConcurrentReference)) {
                    return false;
                }
                RefConcurrentMap.RefConcurrentReference<?> other = (RefConcurrentMap.RefConcurrentReference<?>) obj;
                T thisRef = this.get();
        ...
        private ComputeReference<V> getComputingReference(ReferenceType referenceType) {
            switch (referenceType) {
                case SOFT:
                    return SoftComputeReference.computing(valueQueue);
                case WEAK:
                    return WeakComputeReference.computing(valueQueue);
                case HARD:
                    return HardComputeReference.computing(valueQueue);
                case NONE:
        ...
        private ComputeReference<V> getValueReference(V value, ReferenceType referenceType) {
            ...
            switch (refType) {
                case SOFT:
                    return new SoftComputeReference<>(value, valueQueue);
                case WEAK:
                    return new WeakComputeReference<>(value, valueQueue);
                case HARD:
                    return new HardComputeReference<>(value, valueQueue);
                case NONE:
                    throw new IllegalArgumentException(
                            "NONE reference type should be handled before calling this method");
                default:
                    throw new IllegalArgumentException("Unknown reference type: " + refType);
            }
        }
        ...
        private RefConcurrentReference<K> getKeyReference(K key, ReferenceType referenceType) {
            ...
            switch (refType) {
                case SOFT:
                    return new SoftRefConcurrentReference<>(key, keyQueue);
                case WEAK:
                    return new WeakRefConcurrentReference<>(key, keyQueue);
                case HARD:
                    return new HardRefConcurrentReference<>(key, keyQueue);
                case NONE:
                    throw new IllegalArgumentException(
                            "NONE reference type should be handled before calling this method");
                default:
