...
public final class DefaultMirrorSelector implements MirrorSelector {
     ...
     *                          "external:http:*" and "external:*" can be used to match all (external) repositories,
     *                          prefixing a repo id with an exclamation mark allows to express an exclusion. For example
     *                          "external:*,!central".
     * @param mirrorOfTypes     The content type(s) of remote repositories to mirror, may be {@code null} or empty to
     *                          match any content type. Similar to the repo id specification, multiple types can be
     *                          comma-separated, the wildcard "*" and the "!" negation syntax are supported. For example
     *                          "*,!p2".
