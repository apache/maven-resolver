...
public final class SmartExecutorUtils {
     ...
     * @param strict Is sizing expected to be strict (ie sizing affects some external/public resource).
     * @param namePrefix The thread name prefixes, must not be {@code null).}
     */
    public static SmartExecutor newSmartExecutor(
            Integer tasks, int maxConcurrentTasks, boolean strict, String namePrefix) {
    ...
    public static SmartExecutor smartExecutor(
            RepositorySystemSession session, Integer tasks, int maxConcurrentTasks, boolean strict, String namePrefix) {
        if (tasks == null && maxConcurrentTasks > 1) {
            return (SmartExecutor)
                    session.getData().computeIfAbsent(SmartExecutor.class.getSimpleName() + "-" + namePrefix, () -> {
                        SmartExecutor smartExecutor = newSmartExecutor(null, maxConcurrentTasks, strict, namePrefix);
                        session.addOnSessionEndedHandler(smartExecutor::close);
                        return new SmartExecutor.NonClosing(smartExecutor);
                    });
        } else {
            return newSmartExecutor(tasks, maxConcurrentTasks, strict, namePrefix);
        }
    }
}
