...
`aether.dependencyCollector.pool.weak` | boolean | Flag controlling data pool behavior used by dependency collector: should it use hard or weak references for object pools. By default uses "hard" references (consume more heap, but is faster). Enabling this will make resolver much more memory conservative, at the cost of up to 10% slower collecting dependency speed (system and Java dependent). | `false` | no
