...
`aether.dependencyCollector.impl` | String | The name of the dependency collector implementation to use: depth-first (original) named `df`, and breadth-first (new in 1.8) named `bf`. Both collectors produce equivalent results, but they may differ performance wise, depending on project being applied to. Experiment (and come back to us!) to figure out which one suits you the better. | `"df"` | no
