 ...
 *
 */
public class OkHttpTransporterTest extends HttpTransporterTestSupport
{
    @Override
    protected TransporterFactory newTransporterFactory( RepositorySystemSession session )
    {
        return new OkHttpTransporterFactory();
    }
    ...
    {
        session.getData().set( OkHttpTransporter.OKHTTP_INSTANCE_KEY_PREFIX + repository.getId(), null );
    ...
    public void testClassify()
    {
        ...
        assertEquals( Transporter.ERROR_OTHER, transporter.classify( new OkHttpException( 403 ) ) );
        assertEquals( Transporter.ERROR_NOT_FOUND, transporter.classify( new OkHttpException( 404 ) ) );
    }
}
