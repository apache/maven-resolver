        ...
        extends AbstractTransporter
{
    ...
    {
        if ( error instanceof Java11HttpException
                && ( (Java11HttpException) error ).getStatusCode() == NOT_FOUND )
        {
            ...
            throws Exception
    {
        ...
        {
            throw new Java11HttpException( response.statusCode() );
        }
    }
            ...
            throws Exception
    {
        ...
        while ( true )
        {
            ...
            if ( resume )
            {
                ...
                request.header( IF_UNMODIFIED_SINCE, toHttpDate( new Date(
        ...
        long offset = 0L, length = response.headers().firstValueAsLong( CONTENT_LENGTH ).orElse( -1L );
        if ( resume )
        {
     ...
     * Visible for testing.
     */
    static final String HTTP_INSTANCE_KEY_PREFIX = Java11HttpTransporterFactory.class.getName() + ".http.";
        ...
        try
        {
            return (HttpClient) session.getData().computeIfAbsent( instanceKey, () ->
            {
                ...
                try
                {
                    ...
                    {
                        ProxySelector proxy = ProxySelector.of( new InetSocketAddress( repository.getProxy().getHost(),
